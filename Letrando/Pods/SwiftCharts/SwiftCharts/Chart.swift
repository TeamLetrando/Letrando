...
public struct ChartSettings {
    ...
    public var customClipRect: CGRect?
...
open class ChartView: UIView, UIGestureRecognizerDelegate {
    ...
    @objc func onPinch(_ sender: UIPinchGestureRecognizer) {
        ...
        case .changed:
            ...
            let x = abs(sender.location(in: self).x - sender.location(ofTouch: 1, in: self).x) // swiftlint:disable:this identifier_name
            let y = abs(sender.location(in: self).y - sender.location(ofTouch: 1, in: self).y) // swiftlint:disable:this identifier_name
