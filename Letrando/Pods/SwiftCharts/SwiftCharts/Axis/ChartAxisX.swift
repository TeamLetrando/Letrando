...
open class ChartAxisX: ChartAxis {
    ...
    override func zoom(_ x: CGFloat, y: CGFloat, centerX: CGFloat, centerY: CGFloat, elastic: Bool) { // swiftlint:disable:this identifier_name
    ...
    fileprivate func keepInBoundaries(_ newOriginX: CGFloat, newEndX: CGFloat) {
        ...
        if newEndX - newOriginX > lastScreenInit - firstScreenInit { // new length > original length
            ...
            if offsetOriginX > 0 {
                firstScreen -= offsetOriginX
                lastScreen -= offsetOriginX
            }
