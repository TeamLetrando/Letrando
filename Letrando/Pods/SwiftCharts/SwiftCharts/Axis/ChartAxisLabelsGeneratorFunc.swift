...
open class ChartAxisLabelsGeneratorFunc: ChartAxisLabelsGeneratorBase {
    ...
    let f: (Double) -> [ChartAxisLabel] // swiftlint:disable:this identifier_name

    /// Convenience initializer for function which maps scalar to a single label
    public convenience init(f: @escaping (Double) -> ChartAxisLabel) { // swiftlint:disable:this identifier_name
        self.init(f: {scalar in
            return [f(scalar)]
        })
    }

    public init(f: @escaping (Double) -> [ChartAxisLabel]) { // swiftlint:disable:this identifier_name
        self.f = f
    }
