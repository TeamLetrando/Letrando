...
public extension Zoomable {
    ...
    fileprivate func setContentViewAnchor(_ centerX: CGFloat, centerY: CGFloat) {
        ...
        contentView.transform.tx -= offsetX
        contentView.transform.ty -= offsetY
    }
    ...
    func keepInBoundaries() {
        ...
        if contentView.frame.origin.y > 0 {
            contentView.transform.ty -= contentView.frame.origin.y
        }
        if contentView.frame.maxY < containerFrame.height {
            contentView.transform.ty += (containerFrame.height - contentView.frame.maxY)
        }
        if contentView.frame.origin.x > 0 {
            contentView.transform.tx -= contentView.frame.origin.x
        }
        if contentView.frame.maxX < containerFrame.width {
            contentView.transform.tx += (containerFrame.width - contentView.frame.maxX)
        }
    }
