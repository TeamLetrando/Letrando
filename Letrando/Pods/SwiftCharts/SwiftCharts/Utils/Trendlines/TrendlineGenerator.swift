...
public struct TrendlineGenerator {
    ...
    public static func trendline(_ chartPoints: [ChartPoint]) -> [ChartPoint] {
        ...
        let (sumX, sumY, sumXY, sumXX): (sumX: Double, sumY: Double, sumXY: Double, sumXX: Double) = chartPoints.reduce((sumX: doubleO, sumY: doubleO, sumXY: doubleO, sumXX: doubleO)) {(tuple, chartPoint) in
            ...
            let x: Double = chartPoint.x.scalar // swiftlint:disable:this identifier_name
            let y: Double = chartPoint.y.scalar // swiftlint:disable:this identifier_name
        ...
        let b = (count * sumXY - sumX * sumY) / (count * sumXX - sumX * sumX) // swiftlint:disable:this identifier_name
        let a = (sumY - b * sumX) / count // swiftlint:disable:this identifier_name

        // equation of line: y = a + bx
        func y(_ x: Double) -> Double { // swiftlint:disable:this identifier_name
            return a + b * x
        }
