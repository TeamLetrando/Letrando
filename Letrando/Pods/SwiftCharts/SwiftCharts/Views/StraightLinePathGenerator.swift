...
open class StraightLinePathGenerator: ChartLinesViewPathGenerator {
    ...
    open func generatePath(points: [CGPoint], lineWidth: CGFloat) -> UIBezierPath {
        ...
        if points.count >= 2 {
            ...
            for i in 0..<(points.count - 1) { // swiftlint:disable:this identifier_name
                let p1 = points[i] // swiftlint:disable:this identifier_name
                let p2 = points[i + 1] // swiftlint:disable:this identifier_name
    ...
    open func generateAreaPath(points: [CGPoint], lineWidth: CGFloat) -> UIBezierPath {
        ...
        if let p = points.first { // swiftlint:disable:this identifier_name
            progressline.move(to: p)
        }

        if points.count >= 2 {
            for i in 1..<points.count { // swiftlint:disable:this identifier_name
                let p = points[i] // swiftlint:disable:this identifier_name
