...
open class InfoBubble: UIView {
    ...
    public convenience init(point: CGPoint, size: CGSize, superview: UIView, arrowHeight: CGFloat = 15, contentView: UIView, bgColor: UIColor = UIColor.gray, minSuperviewPadding: CGFloat = 2, space: CGFloat = 12, horizontal: Bool = false) {
        ...
        let w: CGFloat = size.width + (horizontal ? arrowHeight : 0) // swiftlint:disable:this identifier_name
        let h: CGFloat = size.height + (!horizontal ? arrowHeight : 0) // swiftlint:disable:this identifier_name

        let x = horizontal ? point.x : min(max(superview.bounds.minX + minSuperviewPadding, point.x - w / 2), superview.bounds.maxX - w - minSuperviewPadding)  // swiftlint:disable:this identifier_name
        let y = horizontal ? min(max(0 + minSuperviewPadding, point.y - h / 2), superview.bounds.maxY - h - minSuperviewPadding) : point.y  // swiftlint:disable:this identifier_name
