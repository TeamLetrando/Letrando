...
open class ChartPointsViewsLayer<T: ChartPoint, U: UIView>: ChartPointsLayer<T> {
    ...
    open func initViews(_ chart: Chart) {
        ...
        if delayBetweenItems =~ 0 {
            for v in viewsWithChartPoints {addSubview(chart, view: v.view)} // swiftlint:disable:this identifier_name
    ...
    func reloadViews() {
        ...
        for v in viewsWithChartPoints { // swiftlint:disable:this identifier_name
    ...
    override open func chartPointsForScreenLocX(_ x: CGFloat) -> [T] { // swiftlint:disable:this identifier_name
        return filterChartPoints {inXBounds(x, view: $0.view)}
    }

    override open func chartPointsForScreenLocY(_ y: CGFloat) -> [T] { // swiftlint:disable:this identifier_name
    ...
    fileprivate func inXBounds(_ x: CGFloat, view: UIView) -> Bool { // swiftlint:disable:this identifier_name
        return (x > view.frame.origin.x) && (x < (view.frame.origin.x + view.frame.width))
    }
    
    fileprivate func inYBounds(_ y: CGFloat, view: UIView) -> Bool { // swiftlint:disable:this identifier_name
    ...
    func updateForTransform() {
        ...
        case .translate:
            for i in 0..<viewsWithChartPoints.count { // swiftlint:disable:this identifier_name
    ...
    open override func modelLocToScreenLoc(x: Double, y: Double) -> CGPoint { // swiftlint:disable:this identifier_name
        switch mode {
        case .scaleAndTranslate:
