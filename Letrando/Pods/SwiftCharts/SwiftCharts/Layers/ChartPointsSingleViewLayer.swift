...
open class ChartPointsSingleViewLayer<T: ChartPoint, U: UIView>: ChartPointsViewsLayer<T, U> {
    ...
    open func showView(chartPoint: T, chart: Chart) {
        ...
        let index = chartPointsModels.map {
            $0.chartPoint
        }.firstIndex(of: chartPoint)!
        let model: ChartPointLayerModel = ChartPointLayerModel(chartPoint: chartPoint, index: index, screenLoc: screenLoc)// swiftlint:disable:next line_length
