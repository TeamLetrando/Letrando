...
open class ChartDividersLayer: ChartCoordsSpaceLayer {
    ...
    fileprivate func drawLine(context: CGContext, color: UIColor, width: CGFloat, p1: CGPoint, p2: CGPoint) { // swiftlint:disable:this identifier_name
    ...
    override open func chartViewDrawing(context: CGContext, chart: Chart) {
        ...
        if axis == .x || axis == .xAndY {
            for xValue in xValues {
                ...
                let y1 = xAxisLayer.lineP1.y + (xAxisLayer.low ? -settings.end : settings.end) // swiftlint:disable:this identifier_name
                let x2 = xScreenLoc // swiftlint:disable:this identifier_name
                let y2 = xAxisLayer.lineP1.y + (xAxisLayer.low ? settings.start : -settings.start) // swiftlint:disable:this identifier_name
        ...
        if axis == .y || axis == .xAndY {
            for yValue in yValues {
                ...
                let x1 = yAxisLayer.lineP1.x + (yAxisLayer.low ? -settings.start : settings.start) // swiftlint:disable:this identifier_name
                let y1 = yScreenLoc // swiftlint:disable:this identifier_name
                let x2 = yAxisLayer.lineP1.x + (yAxisLayer.low ? settings.end : settings.end) // swiftlint:disable:this identifier_name
                let y2 = yScreenLoc // swiftlint:disable:this identifier_name
                drawLine(context: context, color: settings.linesColor, width: settings.linesWidth, p1: CGPoint(x: x1, y: y1), p2: CGPoint(x: x2, y: y2))
            }
        }
    }
