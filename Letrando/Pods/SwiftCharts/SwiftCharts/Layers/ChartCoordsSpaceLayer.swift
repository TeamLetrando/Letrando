...
open class ChartCoordsSpaceLayer: ChartLayerBase {
    ...
    open func modelLocToScreenLoc(x: Double, y: Double) -> CGPoint {  // swiftlint:disable:this identifier_name
        return CGPoint(x: modelLocToScreenLoc(x: x), y: modelLocToScreenLoc(y: y))
    }
    open func modelLocToScreenLoc(x: Double) -> CGFloat {  // swiftlint:disable:this identifier_name
        return xAxis.innerScreenLocForScalar(x) / (chart?.contentView.transform.a ?? 1)
    }
    open func modelLocToScreenLoc(y: Double) -> CGFloat {  // swiftlint:disable:this identifier_name
        return yAxis.innerScreenLocForScalar(y) / (chart?.contentView.transform.d ?? 1)
    }

    open func modelLocToContainerScreenLoc(x: Double, y: Double) -> CGPoint {  // swiftlint:disable:this identifier_name
        return CGPoint(x: modelLocToContainerScreenLoc(x: x), y: modelLocToContainerScreenLoc(y: y))
    }
    open func modelLocToContainerScreenLoc(x: Double) -> CGFloat { // swiftlint:disable:this identifier_name
        return xAxis.screenLocForScalar(x) - (chart?.containerFrame.origin.x ?? 0)
    }
    open func modelLocToContainerScreenLoc(y: Double) -> CGFloat { // swiftlint:disable:this identifier_name
        return yAxis.screenLocForScalar(y) - (chart?.containerFrame.origin.y ?? 0)
    }

    open func modelLocToGlobalScreenLoc(x: Double, y: Double) -> CGPoint { // swiftlint:disable:this identifier_name
        return CGPoint(x: modelLocToGlobalScreenLoc(x: x), y: modelLocToGlobalScreenLoc(y: y))
    }
    open func modelLocToGlobalScreenLoc(x: Double) -> CGFloat { // swiftlint:disable:this identifier_name
        return xAxis.screenLocForScalar(x)
    }
    open func modelLocToGlobalScreenLoc(y: Double) -> CGFloat { // swiftlint:disable:this identifier_name
        return yAxis.screenLocForScalar(y)
    }
    open func scalarForScreenLoc(x: CGFloat) -> Double { // swiftlint:disable:this identifier_name
        return xAxis.innerScalarForScreenLoc(x * (chart?.contentView.transform.a ?? 1))
    }
    open func scalarForScreenLoc(y: CGFloat) -> Double { // swiftlint:disable:this identifier_name
    ...
    open func contentToGlobalCoordinates(_ point: CGPoint) -> CGPoint? {
        return contentToContainerCoordinates(point).flatMap {
            containerToGlobalCoordinates($0)
        }
    }
