...
open class ChartPointsLineTrackerLayer<T: ChartPoint, U>: ChartPointsLayer<T> {
    ...
    public convenience init(xAxis: ChartAxis, yAxis: ChartAxis, lines: [[T]], lineColor: UIColor, animDuration: Float, animDelay: Float, settings: ChartPointsLineTrackerLayerSettings, positionUpdateHandler: (([ChartTrackerSelectedChartPoint<T, U>]) -> Void)? = nil) {
        self.init(xAxis: xAxis, yAxis: yAxis, lines: lines.map {
                    ChartTrackerLineModel(chartPoints: $0)
        }, lineColor: lineColor, animDuration: animDuration, animDelay: animDelay, settings: settings, positionUpdateHandler: positionUpdateHandler)
    }
    ...
    override func initChartPointModels() {
        ...
        chartPointsModels = Array(lineModels.map {
                            $0.chartPointModels
        }.joined()) // consistency
    }
    ...
    override func updateChartPointsScreenLocations() {
        ...
        for i in 0..<lineModels.count { // swiftlint:disable:this identifier_name
    ...
    fileprivate func findLineIntersection(p0X: CGFloat , p0y: CGFloat, p1x: CGFloat, p1y: CGFloat, p2x: CGFloat, p2y: CGFloat, p3x: CGFloat, p3y: CGFloat) -> CGPoint? {
        var s02x: CGFloat, s02y: CGFloat, s10x: CGFloat, s10y: CGFloat, s32x: CGFloat, s32y: CGFloat, sNumer: CGFloat, tNumer: CGFloat, denom: CGFloat, t: CGFloat; // swiftlint:disable:this identifier_name
        s10x = p1x - p0X
        s10y = p1y - p0y
        s32x = p3x - p2x
        s32y = p3y - p2y
        ...
        // Collision detected
        t = tNumer / denom
        let i_x = p0X + (t * s10x) // swiftlint:disable:this identifier_name
        let i_y = p0y + (t * s10y) // swiftlint:disable:this identifier_name
    ...
    fileprivate func intersectsWithChartPointLines(_ rect: CGRect) -> Bool {
        ...
        return iterateLineSegments({p1, p2, _, _ in // swiftlint:disable:this identifier_name
    ...
    fileprivate func updateTrackerLineOnValidState(updateFunc: (_ view: UIView) -> Void) {
    ...
    fileprivate func iterateLineSegments<V>(_ f: (_ p1: ChartPointLayerModel<T>, _ p2: ChartPointLayerModel<T>, _ lineIndex: Int, _ line: ChartTrackerLineLayerModel<T, U>) -> V?) -> V? { // swiftlint:disable:this identifier_name
        for (index, line) in lineModels.enumerated() {
            ...
            for i in 0..<(lineChartPoints.count - 1) { // swiftlint:disable:this identifier_name
                let m1 = lineChartPoints[i] // swiftlint:disable:this identifier_name
                let m2 = lineChartPoints[i + 1] // swiftlint:disable:this identifier_name
    ...
    fileprivate func updateTrackerLine(touchPoint: CGPoint) {
        updateTrackerLineOnValidState {(view) in
            ...
            let _: Any? = self.iterateLineSegments({p1, p2, lineIndex, lineModel in // swiftlint:disable:this identifier_name
            ...
            if !intersections.isEmpty {
                ...
                if self.chartPointsModels.count > 1 {
                    ...
                    let chartPoints: [ChartTrackerSelectedChartPoint<T, U>] = intersections.map {intersection in
                        ...
                        let x = dummyModel.chartPoint.x.copy(xScalar) // swiftlint:disable:this identifier_name
                        let y = dummyModel.chartPoint.y.copy(yScalar) // swiftlint:disable:this identifier_name
    ...
    open override func modelLocToScreenLoc(x: Double) -> CGFloat { // swiftlint:disable:this identifier_name
        return xAxis.screenLocForScalar(x) - (chart?.containerFrame.origin.x ?? 0)
    }

    open override func modelLocToScreenLoc(y: Double) -> CGFloat { // swiftlint:disable:this identifier_name
    ...
    open override func zoom(_ x: CGFloat, y: CGFloat, centerX: CGFloat, centerY: CGFloat) { // swiftlint:disable:this identifier_name
